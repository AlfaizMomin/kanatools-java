 ...
 *
 * <p>Port of PHP's "mb_convert_kana" function for Java.
...
public class KanaConverter {
   ...
   */
  public static String convertKana(
      String original_string, int conversion_ops, String chars_to_ignore) {
    ...
    while (i < char_count) {
      ...
      //// Source:
      // https://github.com/php/php-src/blob/128eda843f7dff487fff529a384fee3c5494e0f6/ext/mbstring/libmbfl/filters/mbfilter_tl_jisx0201_jisx0208.c#L41
      if (0 != (conversion_ops & OP_HAN_ASCII_TO_ZEN_ASCII)) {
        current_char = convertHankakuAsciiToZenkakuAscii(current_char);
      }

      if (current_char == this_char && 0 != (conversion_ops & OP_HAN_LETTER_TO_ZEN_LETTER)) {
        current_char = convertHankakuLetterToZenkakuLetter(current_char);
      }

      if (current_char == this_char && 0 != (conversion_ops & OP_HAN_NUMBER_TO_ZEN_NUMBER)) {
        current_char = convertHankakuNumberToZenkakuNumber(current_char);
      }

      if (current_char == this_char && 0 != (conversion_ops & OP_HAN_SPACE_TO_ZEN_SPACE)) {
      ...
      if (current_char == this_char
        ...
        if (performed_hankaku_conversion && 0 == (conversion_ops & OP_HAN_KATA_TO_ZEN_KATA)) {
      ...
      if (current_char == this_char && 0 != (conversion_ops & OP_ZEN_ASCII_TO_HAN_ASCII)) {
        current_char = convertZenkakuAsciiToHankakuAscii(current_char);
      }

      if (current_char == this_char && 0 != (conversion_ops & OP_ZEN_LETTER_TO_HAN_LETTER)) {
        current_char = convertZenkakuLetterToHankakuLetter(current_char);
      }

      if (current_char == this_char && 0 != (conversion_ops & OP_ZEN_NUMBER_TO_HAN_NUMBER)) {
        current_char = convertZenkakuNumberToHankakuNumber(current_char);
      }

      if (current_char == this_char && 0 != (conversion_ops & OP_ZEN_SPACE_TO_HAN_SPACE)) {
        current_char = convertZenkakuSpaceToHankakuSpace(current_char);
      }

      if (current_char == this_char && 0 != (conversion_ops & OP_ZEN_KATA_TO_HAN_KATA)) {
      ...
      // Do not enter this block if the current character is a zenkaku katakana character, no matter
      // the flags
      // Protects against katakana characters being incorrectly converted by zen-hiragana to
      // han-katakana logic
      if (current_char == this_char
        ...
        if (0 != (conversion_ops & OP_ZEN_HIRA_TO_HAN_KATA)) {
          // Proceed to convert to hankaku if requested (skip if zen-kata to han-kata conversion was
          // already performed)
          hankaku_diacritic_suffix = determineHankakuDiacriticSuffix(current_char);
          current_char = convertZenkakuKatakanaToHankakuKatakana(current_char);
        }
      ...
      if (current_char == this_char && 0 != (conversion_ops & OP_ZEN_KATA_TO_ZEN_HIRA)) {
   ...
   */
  public static String convertKana(
      String original_string, String conversion_ops_string, String chars_to_ignore) {
