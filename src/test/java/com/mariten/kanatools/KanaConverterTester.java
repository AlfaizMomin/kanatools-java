...
public abstract class KanaConverterTester {
  /**
   * Takes in property (specified in Ant's build.xml) indicating whether to perform actual PHP
   * testing or not
   */
  ...
  public KanaConverterTester() {
    ...
    if (php_option_from_ant != null && php_option_from_ant.equals("yes")) {
   ...
   * @param execute_php_test Also check that these strings match each other using PHP's
   ...
   */
  protected void assertConverted(
      int conv_flags, boolean execute_php_test, String str_to_convert, String expected_result) {
  ...
  protected void assertConverted(
      String conv_flags_string,
      boolean execute_php_test,
      String str_to_convert,
      String expected_result) {
    assertEquals(expected_result, KanaConverter.convertKana(str_to_convert, conv_flags_string));
    if (execute_php_test) {
      int conv_flags = -1;
      try {
        // Translate int-flag op format to PHP-style string op format using restricted function in
        // KanaConverter
        Class<?> target_class = Class.forName("com.mariten.kanatools.KanaConverter");
        Method restricted_converter =
            target_class.getDeclaredMethod("createOpsArrayFromString", String.class);
  ...
  protected void assertConverted(
      String conv_flags_string, String str_to_convert, String expected_result) {
    this.assertConverted(
        conv_flags_string, this.do_direct_php_testing, str_to_convert, expected_result);
  }
   ...
   * @param expected_result Expected results of convertKana function
   */
  protected void assertConvertedWithExclusions(
      int conv_flags, String chars_to_ignore, String str_to_convert, String expected_result) {
    assertEquals(
        expected_result, KanaConverter.convertKana(str_to_convert, conv_flags, chars_to_ignore));
  }
  ...
  /**
   * Perform an actual call to PHP's "mb_convert_kana" function and assert the converted string
   * matches the expected string.
   *
   * @param conv_flags flag-based integer containing conversion methods (will be converted to
   *     PHP-style string)
   ...
   */
  protected void assertConvertedUsingPHP(
      int conv_flags, String str_to_convert, String expected_result) {
    ...
    String[] command_for_php =
        new String[] {
          "php",
          "-r",
          "echo(mb_convert_kana('"
              + str_to_convert_for_php
              + "', '"
              + conv_ops_for_php
              + "', 'UTF-8'));"
        };
    ...
    try {
      ...
      BufferedReader process_output =
          new BufferedReader(new InputStreamReader(php_process.getInputStream()));
